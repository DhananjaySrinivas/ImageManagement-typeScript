...
const Gallery= () => {
    ...
    const searchKey = useSelector((state:any) => state.gallery.searchTerm);
    ...
    const storePhotosFromWeb = () => {
        (async () => {
            ...
            let count =0 ;
            resData.map(({id, height, width, created_at, description, urls: {thumb}}:any) => {
                count = count +1;
                return {
                    id: id,
                    url: thumb,
            ...
            }).forEach((img:any) => dispatch(galleryActions.addToGallery(img)))
        })().catch(err => {
            console.error(err);
        });
    }
    ...
    const filtered = images.filter((img:any)  => img.name.includes(searchKey));
